...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="testng:suiteContentStart">
        <xsl:param name="suiteElement"/>
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                <xsl:call-template name="htmlHead"/>
                <body>
                    <h2 align="center"><font color = "blue"><u>MagentoApp Test Results Overview</u>:</font></h2>
                     <!-- Image file -->
                   <img src ="http://vtrack.vensaiinc.com/vtrack/jquery/css/logo/vensai-logo.png"  width="20%"/>
                    
                    <table style='margin-top:20px;' align="right" width="35%">
                    	<tr>
                        ...
                        <tr>
                            ...
                            <td>
								<xsl:value-of select="format-dateTime((suite[@name != 'Default Suite']/@started-at), '[D01]-[M01]-[Y0001]  [H01]:[m01]:[s01] GMT')"/>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                            	<xsl:value-of select="format-dateTime((suite[@name != 'Default Suite']/@finished-at), '[D01]-[M01]-[Y0001]  [H01]:[m01]:[s01] GMT')"/>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                            	<xsl:value-of select="round((suite[@name != 'Default Suite']/@duration-ms) div 1000)"/> <b>s</b>
                            </td>
                        </tr>
                    ...
                    <table width="100%">
                        <tr>
                            <td align="center" id="chart-container" width="15%">
                                <script type="text/javascript" width="540" >
    ...
    <xsl:template name="navigationFile">
        ...
        <xsl:result-document href="{testng:absolutePath('navigation.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                <xsl:call-template name="htmlHead"/>
                <body>
                    <h2 style="width: 250px;margin-bottom: 5px;">
                    	<font color = "blue"> 
                        	<u> <xsl:value-of select="testng:getVariableSafe($testNgXslt.reportTitle, 'TestNG Results')"/> </u>:
						</font>
                    </h2>
                    <div>
                    ...
                    <!-- <div>
                        <a href="../{$logFile}" target="content"
                           onclick="javscript:clearAllSelections();">Console output
                        </a>
                    </div> -->
                    <div>
                        <a href="../{$severityFile}" target="content"
                           onclick="javscript:clearAllSelections();">Severity Levels
                        </a>
                    </div>
                    <div>
                        <xsl:for-each select="$suiteElements">
    ...
    <xsl:template name="testMethodsList">
        ...
        <xsl:for-each select="$methodList">
            ...
            <tr id="{concat($methodId, '_row')}" class="{testng:testMethodStatus(.)}">
                ...
                <td nowrap="true">
                    <xsl:if test="./exception">
                    	<xsl:variable name="testName" select="concat(@name, '(', testng:trim(testng:concatParams(./params/param)), ')')"/>
                        <a href="../Screenshots/{$testName}.png" target="_blank">ScreenShot</a>
                    </xsl:if>
                    &#160; 
                </td>
            </tr>
